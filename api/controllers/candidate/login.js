...
module.exports = {
  friendlyName: 'Candidate Login',

  description: 'Login for candidate account.',
  ...
  exits: {
    success: {
      description: 'Candidate logged in successfully.'
    },
    badCombo: {
  ...
  fn: async function (inputs, exits) {
    try {
      ...
      let user = await User.findOne({ email: inputs.email.toLowerCase(), role: 'candidate' });
      if (!user) {
      ...
      sails.log.error('Error in candidate/login:', err);
